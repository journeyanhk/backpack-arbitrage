...
### 修复
- ★ 启动对账容差改为市场最小步长，手续费扣币/精度截断产生的尘差（如 XRP 买 9.7 被扣
  0.0097 手续费，真实 9.6903 vs 账本 9.7）自动对齐账本，不再误报"未知敞口禁开仓"
- 超过步长的差异（真幽灵/真实未知敞口）仍会标记并禁止开仓

## [5.1.1] - 2026-08-12

### 修复
- ★ 开仓两腿数量统一到较粗精度（如 XRP 现货 0.1 / 永续 0.0001）：原实现现货 9.7833 被
  交易所截断成 9.7、永续保持 9.7832，产生恒定 0.0832 失衡 → 每次开仓都触发虚假回滚并被
  误报失败；且平仓留尘头（0.0922）同源。现两腿使用同一取整后数量，完全对齐
- 低于最小下单量时明确拒绝开仓并提示加大单笔金额
